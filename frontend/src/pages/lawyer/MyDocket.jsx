    ...
    <div className="p-8 bg-slate-50 dark:bg-[#0B1120] dark:text-slate-100 min-h-screen font-sans transition-colors">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
           <div>
              <h1 className="text-3xl font-bold font-serif-heading text-slate-900 dark:text-white">My Docket</h1>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your active cases and track payments.</p>
           </div>
                 ...
                 <button
                   ...
                   className={`px-5 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all
                     ${activeTab === tab 
                       ...
                       : 'text-slate-500 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-900'}`}
                 >
                   {tab}
                 </button>
              ))}
        ...
        <div className="grid gap-6">
           {loading ? (
              ...
              <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                 ...
                 <p className="text-slate-500 dark:text-slate-400 text-sm">There are no cases in this category.</p>
              </div>
           ) : (
              cases.map((c) => (
                 <div key={c.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-all group">
                    <div className="flex justify-between items-start">
                       <div className="flex gap-4">
                          <div className={`p-3 rounded-xl h-fit ${
                             c.status === 'Payment Pending' ? 'bg-amber-50 text-amber-600' :
                             c.status === 'Active' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300'
                          }`}>
                             {c.status === 'Payment Pending' ? <CreditCard className="w-6 h-6" /> : <ShieldCheck className="w-6 h-6" />}
                          </div>
                          <div>
                             <h3 className="text-xl font-bold text-slate-900 dark:text-white capitalize">{c.title}</h3>
                             <div className="text-sm text-slate-500 dark:text-slate-300 mt-1 flex items-center gap-2">
                                <span className="font-semibold text-slate-700 dark:text-white">{c.users?.full_name}</span>
                                <span>•</span>
                             ...
                             {c.upcoming_event ? (
                                <div className="mt-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded-md w-fit">
                                   <Clock className="w-3 h-3" /> 
                                   Scheduled: {new Date(c.upcoming_event.start_time).toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'})}
                                </div>
                             ) : (
                                <div className="mt-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-300">
                       ...
                       <div className="text-right flex flex-col items-end gap-2">
                          {c.status === 'Payment Pending' ? (
                            ...
                            <span className="text-slate-400 dark:text-slate-300 text-sm font-bold">Closed</span>
                          )}
                       </div>
                    </div>
                 </div>
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
             <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 w-96 shadow-2xl transform transition-all scale-100 border border-slate-100 dark:border-slate-700">
                <div className="flex flex-col items-center text-center">
                   <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4">
                      <CheckCircle className="w-8 h-8 text-green-600" />
                   </div>
                   <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Meeting Scheduled!</h3>
                   <p className="text-slate-500 dark:text-slate-300 text-sm mb-6">
                      Your event has been successfully added to the docket for <span className="font-bold text-slate-800 dark:text-white">{lastScheduledEvent && new Date(lastScheduledEvent.start_time).toLocaleDateString()}</span>.
                   </p>
                   <button 
