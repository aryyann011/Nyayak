...
export default function Schedule() {
  ...
  // --- FETCH & DEDUPLICATE (Wrapped in useCallback to prevent endless loops) ---
  ...
  useEffect(() => {
    ...
    // Improved Realtime Listener
    const subscription = supabase
      .channel('schedule_updates')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'legal_events', filter: `lawyer_id=eq.${user?.id}` }, () => {
          console.log("Realtime event detected. Refetching...");
          fetchEvents();
      })
      .subscribe();
  ...
  const upcoming = useMemo(() => {
      ...
      // Ensure they are sorted so we truly get the 'next' one
  ...
  const handleCreateEvent = async (eventData) => {
    // Note: The AddEventModal already handles Supabase insertion now.
    // We just need to close the modal and let the Realtime listener fetch the new data.
    setShowAdd(false);
    fetchEvents();
  };
  ...
  // 1. DAY VIEW COMPONENT (Fixed to strictly show TODAY)
  const DayView = () => {
    // Filter events to ONLY show those matching today's date
    const todayString = now().toDateString();
    const displayEvents = events.filter(e => e.start.toDateString() === todayString); 
    
    if (displayEvents.length === 0) return (
       <div className="p-10 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
         <p className="text-slate-400 font-bold">No events scheduled for today.</p>
       </div>
    );
        ...
        {days.map((day, i) => (
          <div key={day} className="flex flex-col bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 h-full">
            ...
            <div className="p-2 flex-1 space-y-2 overflow-y-auto">
               {events.filter(e => e.start.getDay() === (i + 1 === 7 ? 0 : i + 1)).map(e => ( // JS getDay() is 0-6 (Sun-Sat)
      ...
      <div className="rounded-xl border border-orange-100 bg-gradient-to-br from-orange-50 to-white dark:bg-slate-800 dark:border-slate-700 p-6 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Court Schedule</div>
              <div className="text-sm text-slate-600 dark:text-slate-400 font-medium">Manage hearings, meetings and deadlines</div>
            </div>
          </div>
            ...
            {['day', 'week', 'month'].map((v) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-bold capitalize rounded-md transition-all ${
                  view === v ? "bg-slate-900 text-white shadow dark:bg-orange-600" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700"
                }`}
                onClick={() => setView(v)}
              >
                {v}
        ...
        {upcoming && upcoming.start && (
           <div className="mt-6 flex items-center gap-3 p-3 bg-white/80 dark:bg-slate-800/80 border border-orange-200/50 dark:border-slate-700 rounded-lg backdrop-blur-sm">
              ...
              <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Next Up:</span>
              <span className="text-sm font-bold text-slate-900 dark:text-white">{upcoming.title}</span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-600">
        ...
        <div className="space-y-6">
           <div className="p-5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                 ...
                 <span className="text-xs font-bold bg-slate-100 dark:bg-slate-700 dark:text-slate-200 px-2 py-1 rounded border border-slate-200 dark:border-slate-600">{Math.round(workload.mins/60)}h Scheduled</span>
              </div>
           ...
           <button 
             ...
             className="w-full py-4 bg-slate-900 dark:bg-orange-600 hover:bg-slate-800 dark:hover:bg-orange-700 text-white rounded-xl font-bold shadow-lg transition-transform active:scale-95"
           >
             + Add Custom Event
           </button>
        </div>
